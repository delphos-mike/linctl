...
Non-trivial features are designed before they are built. Two tracked artifacts
and one untracked scratch area:

- **Specs** — `docs/specs/YYYY-MM-DD-<name>.md`. The *what* and *why*: problem,
  goals/non-goals, command surface, API surface, and follow-ups. Written first.
- **Plans** — `docs/plans/YYYY-MM-DD-<name>.md`. The *how*: ordered,
  commit-sized implementation steps with verification per step. Derived from a
  spec.
- **Notes** — `.local/notes/<plan-id>/` (gitignored). Working scratch:
  introspection dumps, decision logs, throwaway experiments. Never committed.

`<name>` is a short kebab-case slug; `<plan-id>` matches the plan's filename
stem (e.g. `2026-07-14-docs-projects-initiatives`). Use the date the artifact
was created. Keep specs and plans in sync as scope changes.
